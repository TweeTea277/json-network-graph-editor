      ...
      submenu: [
        {
          ...
          onClickFunction: function (event) {
            let target = event.target || event.cyTarget;
            document.getElementById("qmenu-elem-source").value = target.data('id');
          },
        },
        {
          id: 'copy-id-target',
          ...
          onClickFunction: function (event) {
            let target = event.target || event.cyTarget;
            document.getElementById("qmenu-elem-target").value = target.data('id');
          },
        },
      ]
    },
...
cy.on("click", function (event) {
  ...
  if (event.target === cy) {
    ...
    document.getElementById("editor-setting-panel").classList.remove("disabled");
    //インスペクターのノードorエッジ設定のパネルを非表示にする
    document.getElementById("node-or-edge-setting-panel").classList.add("disabled");
...
cy.on('click', 'node, edge', function (event) {
  ...
  document.getElementById("node-or-edge-setting-panel").classList.remove("disabled");
...
document.getElementById("view-png-page-button").addEventListener("click", viewPngPage);
...
document.getElementById("load-text-json-button").addEventListener("click", function () {


});

//図のJSONをファイルから読み込むボタン
document.getElementById("load-json-file").addEventListener("change", event => {
...
const readGraphJson = function (loadJsonData) {
  ...
  const loadingData
    = jsonData.replaceAll(/ {2,}/g, " ").replaceAll(/, *}/g, "}").replaceAll(/} +}/g, "}}").replaceAll(/,\n*]/g, "\n]");
...
document.getElementById("graph-add-text-json-button").addEventListener("click", function () {
...
const graphJsonView = function () {
  const blob = new Blob(
    [document.getElementById("save-json-data").value],
...
document.getElementById("graph-json-view-button").addEventListener("click", graphJsonView);
...
document.getElementById("graph-json-save-txt-button").addEventListener("click", function () {
  graphJsonFileSave(".txt");
});
document.getElementById("graph-json-save-json-button").addEventListener("click", function () {
...
document.getElementById("editor-setting-change-button").addEventListener("click", function () {
...
document.getElementById("rest-qmenu-button").addEventListener("click", function () {
  document.getElementById("qmenu-elem-parent").value = "";
  document.getElementById("qmenu-elem-source").value = "";
  document.getElementById("qmenu-elem-target").value = "";
...
const saveGraphImageButton = document.getElementById("save-json-image-func-button");
...
const inspectorFoldingButton = document.getElementById("inspector-folding-button");
